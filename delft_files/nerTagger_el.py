...
def train(embedding_name, dataset_type='conll2003', lang='en', architecture='BidLSTM_CRF', use_ELMo=False, use_BERT=False, data_path=None): 
    ...
    elif (dataset_type == 'conll2003') and (lang == 'el'):
        print('Loading data...')
        # dataset_type = 'custom'
        # x_all, y_all = load_data_and_labels_conll('data/sequenceLabelling/el_wiki/final_conll_iobes.txt')
        x_all, y_all = load_data_and_labels_conll('data/sequenceLabelling/el_wiki/final_conll_bio2.txt')
        x_train, x_valid, y_train, y_valid = train_test_split(x_all, y_all, test_size=0.1)
        stats(x_train, y_train, x_valid, y_valid)

        model_name = 'ner-el-conll2003'
        if use_ELMo:
            model_name += '-with_ELMo'
        elif use_BERT:
            model_name += '-with_BERT'
    ...
    elif (dataset_type == 'conll2012') and (lang == 'el'):
        print('Loading data...')
        # dataset_type = 'custom'
        # x_all, y_all = load_data_and_labels_conll('data/sequenceLabelling/el_onto5/final_onto5_iobes.txt')
        x_all, y_all = load_data_and_labels_conll('data/sequenceLabelling/el_onto5/final2_onto5.bio2')
        x_train, x_valid, y_train, y_valid = train_test_split(x_all, y_all, test_size=0.1)
        stats(x_train, y_train, x_valid, y_valid)

        model_name = 'ner-el-conll2012'
        if use_ELMo:
            model_name += '-with_ELMo'
        elif use_BERT:
            model_name += '-with_BERT'
...
def train_eval(embedding_name, 
    ...
    if (dataset_type == 'conll2003') and (lang == 'en'):
        ...
        x_train, y_train = load_data_and_labels_conll('data/sequenceLabelling/elNER4/train_elNER4.iobes')
        x_valid, y_valid = load_data_and_labels_conll('data/sequenceLabelling/elNER4/dev_elNER4.iobes')
        x_eval, y_eval = load_data_and_labels_conll('data/sequenceLabelling/elNER4/test_elNER4.iobes')
        # dataset_type = 'custom'
        # x_all, y_all = load_data_and_labels_conll('data/sequenceLabelling/elNER4/final_elNER4_iobes.txt')
        # x_train_all, x_eval, y_train_all, y_eval = train_test_split(x_all, y_all, test_size=0.1)
        # x_train, x_valid, y_train, y_valid = train_test_split(x_train_all, y_train_all, test_size=0.1)
        stats(x_train, y_train, x_valid, y_valid, x_eval, y_eval)
        ...
        if use_ELMo:
            model_name += '-with_ELMo'
            # custom batch size for Greek ELMo
            batch_size = 20
        elif use_BERT:
            # need to find a Greek BERT :/
            model_name += '-with_BERT'
        model_name += '-' + architecture
        ...
        x_train, y_train = load_data_and_labels_conll('data/sequenceLabelling/elNER18/train_elNER18.iobes')
        x_valid, y_valid = load_data_and_labels_conll('data/sequenceLabelling/elNER18/dev_elNER18.iobes')
        x_eval, y_eval = load_data_and_labels_conll('data/sequenceLabelling/elNER18/test_elNER18.iobes')
        # dataset_type = 'custom'
        # x_all, y_all = load_data_and_labels_conll('data/sequenceLabelling/elNER18/final_elNER18_iobes.txt')
        # x_train_all, x_eval, y_train_all, y_eval = train_test_split(x_all, y_all, test_size=0.1)
        # x_train, x_valid, y_train, y_valid = train_test_split(x_train_all, y_train_all, test_size=0.1)
        stats(x_train, y_train, x_valid, y_valid, x_eval, y_eval)
...
if __name__ == "__main__":
    ...
    if action == 'tag':
        if lang is not 'en' and lang is not 'fr' and lang is not 'el':
